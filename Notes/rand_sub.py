...
def duplicate_obj(objects=[], num_copies=1, minX=0, maxX=10, minY=0, maxY=10, minZ=0, maxZ=10, debug=True):
	...
	minZ:
	maxZ:
	debug: if True, prints additional output for troubleshooting
	"""
	...
	for i in range(0, len(objects)):
		for j in range(0, num_copies):
			pprint("copying object {obj} copy number: {copy} of {total_copies} total copies".format(obj=i, copy=j, total_copies=num_copies))
